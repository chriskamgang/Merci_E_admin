...
class LoginController extends BaseLoginController
{
    ...
    // nexah
    public function enable_nexah($mobile, $otp, $country_code)
    {
        $nexahUser = get_sms_settings('nexah_user');
        $nexahPassword = get_sms_settings('nexah_password');
        $nexahSenderId = get_sms_settings('nexah_sender_id');

        $msg = "Votre code OTP Merci E est : $otp";

        // Nexah expects digits only, e.g. 2376XXXXXXXX (no "+", spaces or dashes).
        $dialCode = preg_replace('/\D/', '', (string) $country_code);
        $number = preg_replace('/\D/', '', (string) $mobile);
        if ($dialCode !== '' && ! str_starts_with($number, $dialCode)) {
            $number = $dialCode.$number;
        }

        try {
            $response = Http::timeout(15)->post('https://smsvas.com/bulk/public/index.php/api/v1/sendsms', [
                'user' => $nexahUser,
                'password' => $nexahPassword,
                'senderid' => $nexahSenderId,
                'sms' => $msg,
                'mobiles' => $number,
            ]);
        } catch (\Throwable $e) {
            Log::error('Nexah SMS request failed', ['mobile' => $number, 'error' => $e->getMessage()]);
