...
class TripRequestController extends StripeController
{
    ...
    {
        // API route (api/v1/user/download-invoice): only the passenger, assigned driver or fleet owner.
        $requestmodel = RequestModel::where('id', $id)->visibleTo(auth()->user())->get();

        if ($requestmodel->isEmpty()) {
            return response()->json(['success' => false, 'message' => 'request_not_found'], 404);
        }
        // dd($request->all());
        $requestmodel = fractal($requestmodel, new TripRequestTransformer)->parseIncludes(['userDetail', 'driverDetail', 'requestBill', 'rejectedDrivers'])->toArray();
        $logo = Setting::where('name', 'logo')->first();
